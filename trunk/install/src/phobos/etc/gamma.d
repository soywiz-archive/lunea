 ...
 *
 * Placed into the public domain by Don Clugston.
Macros:
...
// Test some high-precision values (50 decimal digits)
  ...
  assert(feqrel(etcgamma.tgamma(1.0/3.L),  2.67893853470774763365569294097467764412868937795730L)>= real.mant_dig-2);
  assert(feqrel(etcgamma.tgamma(0.25L),
  3.62560990822190831193068515586767200299516768288006)>= real.mant_dig-1);
  assert(feqrel(etcgamma.tgamma(1.0/5.0L),
   ...
   // TODO: test derivatives as well.
    for (int i=0; i<testpoints.length; i+=3) {
         ...
         if (testpoints[i]<MAXGAMMA) {
            //writefln("%a  %a  ", log(fabs(tgamma(testpoints[i]))), testpoints[i+1], feqrel(lgamma(testpoints[i]), testpoints[i+1]));
         assert( feqrel(log(fabs(tgamma(testpoints[i]))), testpoints[i+1]) > real.mant_dig-5);
         }
     }
     assert(lgamma(-50.2)==log(fabs(tgamma(-50.2))));
//   writefln("%a %a", lgamma(-38.8), log(fabs(tgamma(-38.8))));
