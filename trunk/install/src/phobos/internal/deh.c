...
EXCEPTION_DISPOSITION __cdecl _local_except_handler(EXCEPTION_RECORD *ExceptionRecord, void *EstablisherFrame, void *ContextRecord, void *DispatcherContext);
...
#define MAKE_EXCEPTION_CODE(severity, facility, exception) (((severity) << 30) | (1 << 29) | (0 << 28) | ((facility) << 16) | (exception))
...
EXCEPTION_DISPOSITION _d_framehandler(EXCEPTION_RECORD *exception_record, struct DEstablisherFrame *frame, CONTEXT context, void *dispatcher_context) {
...
int _d_exception_filter(struct _EXCEPTION_POINTERS *eptrs, int retval, Object **exception_object) {
...
Object *_d_translate_se_to_d_exception(EXCEPTION_RECORD *exception_record) {
    ...
    switch (exception_record->ExceptionCode) {
        ...
        case STATUS_DIGITAL_MARS_D_EXCEPTION: pti = (Object *)(exception_record->ExceptionInformation[0]); break;
        case STATUS_INTEGER_DIVIDE_BY_ZERO:   pti = _d_create_exception_object(&_Class_9Exception, "Integer Divide by Zero"); break;
        case STATUS_FLOAT_DIVIDE_BY_ZERO:     pti = _d_create_exception_object(&_Class_9Exception, "Float Divide by Zero"); break;
        case STATUS_ACCESS_VIOLATION:         pti = _d_create_exception_object(&_Class_9Exception, "Access Violation"); break;
        case STATUS_STACK_OVERFLOW:           pti = _d_create_exception_object(&_Class_9Exception, "Stack Overflow"); break;
        // convert all other exception codes into a Win32Exception
        default: pti = _d_create_exception_object(&_Class_9Exception, "Win32 Exception"); break;
    }
...
EXCEPTION_DISPOSITION _d_monitor_handler(EXCEPTION_RECORD *exception_record, struct DEstablisherFrame *frame, CONTEXT context, void *dispatcher_context) {
    if (exception_record->ExceptionFlags & EXCEPTION_UNWIND) {
...
void __stdcall _d_throw(Object *h) {
	// ****************************************
	// LUNEA PATCH to avoid throwing exceptions
	// ****************************************
	return;

    //printf("_d_throw(h = %p, &h = %p)\n", h, &h);
    //printf("\tvptr = %p\n", *(void **)h);
    RaiseException(STATUS_DIGITAL_MARS_D_EXCEPTION, EXCEPTION_NONCONTINUABLE, 1, (DWORD *)&h);
}
